...
public class JobsCommands implements CommandExecutor {
	...
	public boolean top(CommandSender sender, String[] args) {
		...
		List<String> FullList = Jobs.getJobsDAO().toplist(args[0]);
		if (FullList.size() <= 0) {
			player.sendMessage(ChatColor.RED + Language.getMessage("command.top.error.nojob"));
			return false;
		}
		player.sendMessage(Language.getMessage("command.top.output.topline").replace("%jobname%", args[0]));
		int i = 0;
		for (String Onename : FullList) {
			i++;
			String[] Splited = Onename.split("%");
			player.sendMessage(Language.getMessage("command.top.output.list").replace("%number%", String.valueOf(i)).replace("%playername%", Splited[0]).replace("%level%", Splited[1]).replace("%exp%", Splited[2]));
		}
		return true;
	}
