...
public class JobsConfiguration {
	...
	public static boolean fixAtMaxLevel, TitleChangeChat, TitleChangeActionBar,
			LevelChangeChat, LevelChangeActionBar, SoundLevelupUse,
			SoundTitleChangeUse;
	public static Integer levelLossPercentage, SoundLevelupVolume,
			SoundLevelupPitch, SoundTitleChangeVolume, SoundTitleChangePitch;
	public static double BoostExp = 1.0;
	public static double BoostMoney = 1.0;
	public static String SoundLevelupSound, SoundTitleChangeSound;
	...
	private synchronized void loadGeneralSettings() {
		...
		writer.addComment("use-block-move-protection", "Experimental mode!.", "Enable blocks, like ore, protection.", "Only enable if you want to protect block from beying moved by pistons.");
		config.addDefault("use-block-move-protection", false);

		writer.addComment("use-block-timer", "Experimental mode!.", "Enable blocks timer protection.", "Only enable if you want to protect block from beying broken to fast.");
		...
		writer.addComment("use-new-enchant-pay-system", "Rewamped enchant system.", "With this turned on, you will get paid for each enchanment by its lvl, not for item used to enchant.");
		config.addDefault("use-new-enchant-pay-system", false);

		writer.addComment("boost", "Money exp boost with special permision.", "You will need to add special permision for groups or players to have money/exp boost.", "Use: jobs.boost.[jobname].money or jobs.boost.[jobname].exp or jobs.boost.[jobname].both for both of them", "1.25 means that player will get 25% more than others, you can set less than 1 to get less from anothers");
		...
		writer.addComment("ShowActionBars", "Action bars", "You can enable/disable message show for players on title change or on levelup");
		...
		config.addDefault("Sounds.LevelUp.use", true);
		config.addDefault("Sounds.LevelUp.sound", "LEVEL_UP");
		config.addDefault("Sounds.LevelUp.volume", 1);
		config.addDefault("Sounds.LevelUp.pitch", 3);
		config.addDefault("Sounds.TitleChange.use", true);
		config.addDefault("Sounds.TitleChange.sound", "LEVEL_UP");
		config.addDefault("Sounds.TitleChange.volume", 1);
		config.addDefault("Sounds.TitleChange.pitch", 3);
		...
		SoundLevelupUse = config.getBoolean("Sounds.LevelUp.use");
		SoundLevelupSound = config.getString("Sounds.LevelUp.sound");
		SoundLevelupVolume = config.getInt("Sounds.LevelUp.volume");
		SoundLevelupPitch = config.getInt("Sounds.LevelUp.pitch");
		SoundTitleChangeUse = config.getBoolean("Sounds.TitleChange.use");
		SoundTitleChangeSound = config.getString("Sounds.TitleChange.sound");
		SoundTitleChangeVolume = config.getInt("Sounds.TitleChange.volume");
		SoundTitleChangePitch = config.getInt("Sounds.TitleChange.pitch");
		...
		copySetting(config, writer, "Sounds.LevelUp.use");
		copySetting(config, writer, "Sounds.LevelUp.sound");
		copySetting(config, writer, "Sounds.LevelUp.volume");
		copySetting(config, writer, "Sounds.LevelUp.pitch");
		copySetting(config, writer, "Sounds.TitleChange.use");
		copySetting(config, writer, "Sounds.TitleChange.sound");
		copySetting(config, writer, "Sounds.TitleChange.volume");
		copySetting(config, writer, "Sounds.TitleChange.pitch");
	...
	private synchronized void loadRestrictedBlocks() {
		...
		conf.options().copyDefaults(true);
		StringBuilder header = new StringBuilder();
		header.append("restrictedblocks - All block to be protected from place/block exploit and piston move").append(System.getProperty("line.separator")).append("Dont count in vegetables").append(System.getProperty("line.separator"));
		...
		header.append("blockstimer - Block protected by timer in sec").append(System.getProperty("line.separator")).append("141-60 means that carrot can be harvested after 60 sec (remember to use id's from placed objects, not from your inventory)").append(System.getProperty("line.separator"));
