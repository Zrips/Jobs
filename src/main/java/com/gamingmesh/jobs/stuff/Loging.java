...
public class Loging {
	...
	public void recordToLog(JobsPlayer jPlayer, String ActionName, String item, double amount, double expAmount) {
		...
		if (jPlayer.getLog().size() > 0
				&& Jobs.getScheduleManager().getDateByInt() != jPlayer.getLog().get(0).getDate()) {
	...
	public void loadToLog(JobsPlayer jPlayer, String ActionName, String item, int count, double money,
			double expAmount) {
		List<Log> logList = jPlayer.getLog();
		boolean found = false;
		for (Log one : logList) {
