...
public class GeneralConfigManager {
	...
	public boolean PayForRenaming, PayForEachCraft, SignsEnabled, SignsColorizeJobName, ShowToplistInScoreboard,
			useGlobalTimer, useCoreProtect, BlockPlaceUse, EnableAnounceMessage, useBlockPiston, useSilkTouchProtection,
			UseCustomNames, UseJobsBrowse, PreventSlimeSplit, PreventMagmaCubeSplit, WaterBlockBreake;
	public int globalblocktimer, CowMilkingTimer, CoreProtectInterval, BlockPlaceInterval, InfoUpdateInterval;
	public Double payNearSpawnerMultiplier, VIPpayNearSpawnerMultiplier, TreeFellerMultiplier, gigaDrillMultiplier,
			superBreakerMultiplier, PetPay, VipPetPay;
	...
	public boolean fixAtMaxLevel, ToggleActionBar, TitleChangeChat, TitleChangeActionBar, LevelChangeChat,
			...
			EmptyServerAcountActionBar, ActionBarsMessageByDefault, ShowTotalWorkers, ShowPenaltyBonus,
			useDynamicPayment, useGlobalBoostScheduler, JobsGUIOpenOnBrowse, JobsGUIShowChatBrowse,
			JobsGUISwitcheButtons, JobsGUIOpenOnJoin;
	...
	/**
	 * Function that tells if the player should be paid while exploring and
	 * flying
	 * 
	 * @return true - pay
	 * @return false - do not
	...
	/**
	 * Function to check if jobs should be hidden to players that lack
	 * permission to join the job
	 * 
	 * @return
	 */
	...
	private synchronized void loadGeneralSettings() {
		...
		c.getW().addComment("locale-language", "Default language.", "Example: en, ru",
		...
		c.getW().addComment("MultiServerCompatability",
		...
		c.getW().addComment("Optimizations.AutoJobJoin.Delay",
		...
		c.getW().addComment("Optimizations.UseLocalOfflinePlayersData",
		...
		c.getW().addComment("Optimizations.DisabledWorlds.Use",
		...
		DisabledWorldsList = c.getStringList("Optimizations.DisabledWorlds.List",
				Arrays.asList(Bukkit.getWorlds().get(0).getName()));

		// c.getW().addComment("Optimizations.Purge.Use", "By setting this to
		// true, Jobs plugin will clean data base on startup from all jobs with
		// level 1 and at 0 exp");
		// PurgeUse = c.get("Optimizations.Purge.Use", false);

		c.getW().addComment("Logging.Use",
		...
		c.getW().addComment("broadcast.on-skill-up.use",
		...
		c.getW().addComment("broadcast.on-level-up.levels",
		...
		c.getW().addComment("hide-jobs-without-permission",
		...
		c.getW().addComment("hide-jobsinfo-without-permission",
		...
		c.getW().addComment("VIP-pay-near-spawner-multiplier",
		...
		c.getW().addComment("enable-pay-for-exploring-when-flying",
		...
		c.getW().addComment("economy-batch-delay",
		...
		c.getW().addComment("economy-async", "Enable async economy calls.",
		...
		c.getW().addComment("Economy.DynamicPayment.use",
		...
		String maxExpEquationInput = c.get("Economy.DynamicPayment.equation",
				"((totalworkers / totaljobs) - jobstotalplayers)/10.0");
		try {
			...
			Bukkit.getConsoleSender().sendMessage(
					ChatColor.RED + "Dynamic payment equation has an invalid property. Disabling feature!");
		...
		c.getW().addComment("Economy.UseServerAcount", "Server economy acount",
		...
		c.getW().addComment("Economy.Taxes.AccountName",
		...
		// Money limit
		c.getW().addComment("Economy.Limit.Money", "Money gain limit",
		...
		c.getW().addComment("Economy.Limit.Money.TimeLimit",
				"Time in seconds: 60 = 1min, 3600 = 1 hour, 86400 = 24 hours");
		MoneyTimeLimit = c.get("Economy.Limit.Money.TimeLimit", 3600);
		c.getW().addComment("Economy.Limit.Money.AnnouncmentDelay",
		...
		c.getW().addComment("Economy.Limit.Point.TimeLimit",
				"Time in seconds: 60 = 1min, 3600 = 1 hour, 86400 = 24 hours");
		...
		// Exp limit
		c.getW().addComment("Economy.Limit.Exp", "Exp gain limit",
		...
		c.getW().addComment("Economy.Limit.Exp.TimeLimit",
				"Time in seconds: 60 = 1min, 3600 = 1 hour, 86400 = 24 hours");
		...
		c.getW().addComment("Economy.Repair.PayForRenaming",
		...
		c.getW().addComment("Economy.MilkingCow.CancelMilking",
		...
		c.getW().addComment("ExploitProtections.General.PlaceAndBreakProtection",
				...
				"This works only until server restart", "Modify restrictedBlocks.yml for blocks you want to protect");
		...
		c.getW().addComment("ExploitProtections.General.StopPistonBlockMove",
		...
		c.getW().addComment("ExploitProtections.General.GlobalBlockTimer",
		...
		c.getW().addComment("ExploitProtections.General.PetPay",
				"Do you want to pay when players pet kills monster/player", "Can be exploited with mob farms",
		...
		c.getW().addComment("ExploitProtections.McMMO.TreeFellerMultiplier",
		...
		c.getW().addComment("ExploitProtections.McMMO.gigaDrillMultiplier",
		...
		c.getW().addComment("ExploitProtections.McMMO.superBreakerMultiplier",
		...
		c.getW().addComment("ExploitProtections.MythicMobs", "MythicMobs plugin support",
		...
		c.getW().addComment("ExploitProtections.Spawner.PreventSlimeSplit",
		...
		c.getW().addComment("ExploitProtections.Spawner.PreventMagmaCubeSplit",
		...
		c.getW().addComment("old-job", "Old job save",
		...
		c.getW().addComment("ActionBars.Messages.EnabledByDefault",
		...
		if (Jobs.getActionBar().getVersion() < 1900) {
			...
			Bukkit.getConsoleSender().sendMessage(
					ChatColor.RED + "[Jobs] Your server version don't support BossBar. This feature will be disabled");
		}

		c.getW().addComment("BossBar.Messages.EnabledByDefault",
		...
		c.getW().addComment("ShowChatMessage", "Chat messages",
		...
		c.getW().addComment("Scoreboard.ShowToplist",
		...
		c.getW().addComment("JobsBrowse.ShowTotalWorkers",
		...
		c.getW().addComment("JobsBrowse.ShowPenaltyBonus",
		...
		c.getW().addComment("JobsGUI.ShowChatBrowse",
		...
		c.getW().addComment("JobsGUI.SwitcheButtons",
		...
		// writer.addComment("Gui.UseJobsBrowse", "Do you want to use jobs
		// browse gui instead of chat text");
