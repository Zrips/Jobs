...
public class ConfigManager {
	...
	private void loadJobSettings() throws IOException {
		...
		// conf.options().header(new StringBuilder().append("Jobs
		// configuration.").append(System.getProperty("line.separator")).append(System.getProperty("line.separator")).append("Stores
		// information about each
		// job.").append(System.getProperty("line.separator")).append(System.getProperty("line.separator")).append("For
		// example configurations, visit
		// http://dev.bukkit.org/bukkit-plugins/jobs-reborn/.").append(System.getProperty("line.separator")).toString());
		...
		for (String jobKey : jobsSection.getKeys(false)) {
			...
			if (jobSection.contains("ChatColour")) {
				...
				if (color == null) {
					...
					Jobs.getPluginLogger()
							.warning("Job " + jobKey + " has an invalid ChatColour property.  Defaulting to WHITE!");
				}
			}
			...
			try {
				...
				Jobs.getPluginLogger().warning(
						"Job " + jobKey + " has an invalid experience-progression-equation property. Skipping job!");
				continue;
			}
			...
			if (jobSection.isString("points-progression-equation")) {
				...
				try {
					...
					Jobs.getPluginLogger().warning(
							"Job " + jobKey + " has an invalid points-progression-equation property. Skipping job!");
					continue;
				}
			}
			...
			if (jobSection.contains("Gui")) {
				...
				if (guiSection.contains("Id") && guiSection.contains("Data") && guiSection.isInt("Id")
					...
					Jobs.getPluginLogger().warning(
							"Job " + jobKey + " has an invalid Gui property. Please fix this if you want to use it!");
			}
			...
			Job job = new Job(jobName, jobShortName, description, color, maxExpEquation, displayMethod, maxLevel,
					vipmaxLevel, maxSlots, jobPermissions, jobCommand, jobConditions, jobItems, jobLimitedItems,
					JobsCommandOnJoin, JobsCommandOnLeave, GUIitem);
				...
				if (typeSection != null) {
					for (String key : typeSection.getKeys(false)) {
						...
						if (material != null) {
							...
							// START HACK
							/*
							 * Historically, GLOWING_REDSTONE_ORE would ONLY
							 * work as REDSTONE_ORE, and putting
							 * GLOWING_REDSTONE_ORE in the configuration would
							 * not work. Unfortunately, this is completely
							 * backwards and wrong.
							 * 
							 * To maintain backwards compatibility, all
							 * instances of REDSTONE_ORE should normalize to
							 * GLOWING_REDSTONE_ORE, and warn the user to change
							 * their configuration. In the future this hack may
							 * be removed and anybody using REDSTONE_ORE will
							 * have their configurations broken.
							 */
							if (material == Material.REDSTONE_ORE && actionType == ActionType.BREAK) {
								Jobs.getPluginLogger().warning(
										"Job " + jobKey + " is using REDSTONE_ORE instead of GLOWING_REDSTONE_ORE.");
								Jobs.getPluginLogger().warning(
						...
						if (type == null) {
							Jobs.getPluginLogger().warning("Job " + jobKey + " has an invalid " + actionType.getName()
									+ " type property: " + key + "!");
							continue;
						}
						...
						if (section.isInt("until-level")) {
							...
							if (untilLevel < fromlevel) {
								Jobs.getPluginLogger()
										.warning("Job " + jobKey + " has an invalid until-level in "
												+ actionType.getName() + " for type property: " + key
												+ "! It will be not set.");
								untilLevel = -1;
							}
						}
						...
						jobInfo.add(new JobInfo(actionType, id, meta, type + subType, income, incomeEquation,
								experience, expEquation, pointsEquation, points, fromlevel, untilLevel));
					}
				}
