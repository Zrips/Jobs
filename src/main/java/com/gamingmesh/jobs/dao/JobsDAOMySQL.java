...
public class JobsDAOMySQL extends JobsDAO {
	...
	public static JobsDAOMySQL initialize(String hostname, String database, String username, String password,
			String prefix) {
	...
	protected synchronized void checkUpdate9() throws SQLException {
		...
		try {
			// Check for jobs table
			tempPrest = conn.prepareStatement(
					"SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = ? AND table_name = ?;");
		...
		if (rows == 0) {
			...
			try {
				prestJobs = conn
						.prepareStatement("SELECT DISTINCT(player_uuid),username FROM " + getPrefix() + "jobs;");
			...
			try {
				prestArchive = conn
						.prepareStatement("SELECT DISTINCT(player_uuid),username FROM " + getPrefix() + "archive;");
			...
			try {
				prestArchiveT = conn.prepareStatement(
						"UPDATE `" + getPrefix() + "archive` SET `userid` = ? WHERE `player_uuid` = ?;");
				conn.setAutoCommit(false);
				for (Entry<String, PlayerInfo> users : tempPlayerMap.entrySet()) {
			...
			try {
				executeSQL(
						"ALTER TABLE `" + getPrefix() + "archive` DROP COLUMN `player_uuid`, DROP COLUMN `username`;");
			} catch (Exception e) {
			}
		}
	}
