...
public class JobsDAOSQLite extends JobsDAO {
		...
		if (rows == 0) {
			...
			try {
				executeSQL("CREATE TABLE `" + getPrefix()
						+ "config` (`key` varchar(50) NOT NULL PRIMARY KEY, `value` varchar(100) NOT NULL);");

				insert = conn
						.prepareStatement("INSERT INTO `" + getPrefix() + "config` (`key`, `value`) VALUES (?, ?);");
	...
	protected synchronized void checkUpdate6() throws SQLException {
		...
		try {
			executeSQL("INSERT INTO `" + getPrefix()
					+ "log_temp` SELECT `id`, `player_uuid`, `username`, `time`, `action`, `itemname`, `count`, `money`, `exp` FROM `"
					+ getPrefix() + "log`;");
		} catch (Exception e) {
		}
	...
	protected synchronized void checkUpdate9() throws SQLException {
		...
		try {
			...
			while (res2.next()) {
				Bukkit.getConsoleSender()
						.sendMessage(res2.getString("player_uuid") + " -> " + res2.getString("username"));
		...
		try {
			...
			while (res3.next()) {
				...
				Bukkit.getConsoleSender()
						.sendMessage(res3.getString("player_uuid") + " -> " + res3.getString("username"));
			}
		} finally {
			close(res3);
			close(prestArchive);
		...
		try {
			prestUsers = conn.prepareStatement(
					"INSERT INTO `" + getPrefix() + "users` (`player_uuid`, `username`) VALUES (?, ?);");
			conn.setAutoCommit(false);
		...
		try {
			...
			while (res4.next()) {
				tempPlayerMap.put(res4.getString("player_uuid"),
						new PlayerInfo(res4.getString("username"), res4.getInt("id")));
			}
		} finally {
			close(res4);
			close(prestUsers2);
		...
		try {
			prestJobsT = conn
					.prepareStatement("UPDATE `" + getPrefix() + "jobs` SET `userid` = ? WHERE `player_uuid` = ?;");
			conn.setAutoCommit(false);
		...
		while (rs11.next()) {
			...
			if (uuid != null) {
				insert11 = conn.prepareStatement("INSERT INTO `" + getPrefix()
						+ "jobs_temp` (`userid`, `job`, `experience`, `level`) VALUES (?, ?, ?, ?);");
		...
		try {
			prestArchiveT = conn
					.prepareStatement("UPDATE `" + getPrefix() + "archive` SET `userid` = ? WHERE `player_uuid` = ?;");
			conn.setAutoCommit(false);
			for (Entry<String, PlayerInfo> users : tempPlayerMap.entrySet()) {
		...
		try {
			...
			while (rsArchive.next()) {
				...
				if (uuid != null) {
					insertArchive = conn.prepareStatement("INSERT INTO `" + getPrefix()
							+ "archive_temp` (`userid`, `job`, `experience`, `level`) VALUES (?, ?, ?, ?);");
		...
		try {
			...
			prestPreLog = conn
					.prepareStatement("UPDATE `" + getPrefix() + "log` SET `userid` = ? WHERE `player_uuid` = ?;");
			conn.setAutoCommit(false);
			for (Entry<String, PlayerInfo> users : tempPlayerMap.entrySet()) {
