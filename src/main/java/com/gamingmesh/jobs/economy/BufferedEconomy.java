...
public class BufferedEconomy {
	...
	private final Map<UUID, BufferedPayment> paymentCache = Collections
			.synchronizedMap(new HashMap<UUID, BufferedPayment>());
	...
	public void payAll() {
		...
		synchronized (paymentCache) {
			...
			if (Jobs.getGCManager().UseTaxes && Jobs.getGCManager().TransferToServerAccount
					&& ServerTaxesAccount != null) {
				economy.depositPlayer(ServerTaxesAccount, TaxesAmount);

				if (ServerTaxesAccount.isOnline()) {
					if (!Jobs.getActionbarToggleList().containsKey(ServerTaxesAccountname)
							&& Jobs.getGCManager().ActionBarsMessageByDefault)
						Jobs.getActionbarToggleList().put(ServerTaxesAccountname, true);
					if (Jobs.getActionbarToggleList().containsKey(ServerTaxesAccountname)
							&& Jobs.getActionbarToggleList().get(ServerTaxesAccountname)) {
						Jobs.getActionBar().send(Bukkit.getPlayer(ServerAccountname), Jobs.getLanguage()
								.getMessage("message.taxes", "[amount]", (int) (TotalAmount * 100) / 100.0));
					}
				}
			}
			...
			for (BufferedPayment payment : paymentCache.values()) {
				...
				// JobsPayment event
				JobsPaymentEvent JobsPaymentEvent = new JobsPaymentEvent(payment.getOfflinePlayer(),
						payment.getAmount(), payment.getPoints());
				...
				if (Jobs.getGCManager().UseServerAccount) {
					if (!hasMoney) {
						Jobs.getActionBar().send(payment.getOfflinePlayer().getPlayer(),
								ChatColor.RED + Jobs.getLanguage().getMessage("economy.error.nomoney"));
						continue;
					}
					if (Jobs.getGCManager().isEconomyAsync())
						Bukkit.getScheduler().runTaskLaterAsynchronously(plugin,
								new BufferedPaymentTask(this, economy, payment), i);
					else
				...
				try {
					...
					if (payment.getOfflinePlayer().isOnline()) {
						JobsPlayer jPlayer = Jobs.getPlayerManager()
								.getJobsPlayer(payment.getOfflinePlayer().getName());
