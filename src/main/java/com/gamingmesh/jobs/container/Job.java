...
public class Job {
	 ...
	 *            - the name of the job
	 * @param jobShortName
	 *            - the shortened version of the name of the job.
	 * @param description
	 *            - a short description of the job.
	 * @param jobColour
	 *            - the colour of the job title as displayed in chat.
	 * @param maxExpEquation
	 *            - the equation by which the exp needed to level up is
	 *            calculated
	 * @param displayMethod
	 *            - the display method for this job.
	 * @param maxLevel
	 *            - the maximum level allowed (null for no max level)
	 * @param vipmaxLevel
	 *            - the maximum vip level allowed (null for no max level)
	 * @param maxSlots
	 *            - the maximum number of people allowed to have this job at one
	 *            time (null for no limits)
	 * @param jobPermissions
	 *            - permissions gained for having the job
	 * @param jobCommands
	 *            - commands to perform on levelup
	 * @param jobItems
	 *            - items with boost
	 * @param jobLimitedItems
	 *            - limited items by lvl
	 * @param CmdOnJoin
	 *            - commands performed on player join
	 * @param CmdOnLeave
	 *            - commands performed on player leave
	 * @param jobConditions
	 *            - jobs conditions
	 */
	public Job(String jobName, String jobShortName, String description, ChatColor jobColour, Parser maxExpEquation,
			DisplayMethod displayMethod, int maxLevel, int vipmaxLevel, Integer maxSlots,
			List<JobPermission> jobPermissions, List<JobCommands> jobCommands, List<JobConditions> jobConditions,
			List<JobItems> jobItems, List<JobLimitedItems> jobLimitedItems, List<String> CmdOnJoin,
			List<String> CmdOnLeave, ItemStack GUIitem) {
	...
	public JobInfo getJobInfo(ActionInfo action, int level) {
		for (JobInfo info : getJobInfo(action.getType())) {
			if (info.getName().equalsIgnoreCase(action.getName())
					|| info.getName().equalsIgnoreCase(action.getNameWithSub())) {
