	...
	public Inventory CreateJobsGUI(Player player) {
		...
		for (int i = 0; i < JobsList.size(); i++) {
			...
			if (Jobs.getGCManager().ShowTotalWorkers)
				Lore.add(Jobs.getLanguage().getMessage("command.browse.output.totalWorkers", "[amount]",
						job.getTotalPlayers()));

			if (Jobs.getGCManager().useDynamicPayment && Jobs.getGCManager().ShowPenaltyBonus)
				if (job.getBonus() < 0)
					Lore.add(Jobs.getLanguage().getMessage("command.browse.output.penalty", "[amount]",
							(int) (job.getBonus() * 100) / 100.0 * -1));
				else
					Lore.add(Jobs.getLanguage().getMessage("command.browse.output.bonus", "[amount]",
							(int) (job.getBonus() * 100) / 100.0));

			Lore.addAll(Arrays.asList(job.getDescription().split("/n")));

			if (job.getMaxSlots() != null)
				Lore.add(Jobs.getLanguage().getMessage("command.info.gui.leftSlots")
						+ ((job.getMaxSlots() - Jobs.getUsedSlots(job)) > 0
								? (job.getMaxSlots() - Jobs.getUsedSlots(job)) : 0));
			...
			for (ActionType actionType : ActionType.values()) {
				...
				if (info != null && !info.isEmpty()) {
					Lore.add(ChatColor.translateAlternateColorCodes('&', "&e" + Jobs.getLanguage()
							.getMessage("command.info.output." + actionType.getName().toLowerCase() + ".info")));
				}
			}
	...
	public Inventory CreateJobsSubGUI(Player player, Job job) {
		...
		for (ActionType actionType : ActionType.values()) {
			...
			for (int z = 0; z < info.size(); z++) {
				...
				Lore.add(ChatColor.translateAlternateColorCodes('&', "&7" + itemName + " " + xpColor + xpString + " "
						+ incomeColor + Jobs.getEconomy().format(income)));
