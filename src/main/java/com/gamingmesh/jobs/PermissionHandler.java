...
public class PermissionHandler {
	...
	public void recalculatePermissions(JobsPlayer jPlayer) {
		...
		// Permissions should only apply if we have permission to use jobs in
		// this world
		if (hasWorldPermission(player, player.getWorld().getName())) {
					...
					for (JobPermission perm : job.getPermissions()) {
						if (perm.getLevelRequirement() <= 0) {
							if (perm.isValue()) {
								 ...
								 * This is in case we already have a true node
								 * there
								 */
				...
				for (JobProgression prog : progression) {
					for (JobPermission perm : prog.getJob().getPermissions()) {
						if (prog.getLevel() >= perm.getLevelRequirement()) {
							/*
							 * If the key exists, don't put a false node in This
							 * is in case we already have a true node there
							 */
							if (perm.isValue()) {
								permissions.put(perm.getNode(), true);
							} else {
			...
			if (permissions.size() > 0) {
				this.plugin.getServer().getPluginManager()
						.addPermission(new Permission(permName, PermissionDefault.FALSE, permissions));
				changed = true;
			}
		}
