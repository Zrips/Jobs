...
public class JobsCommands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (sender instanceof Player) {
			if (!Jobs.getGCManager().canPerformActionInWorld(((Player) sender).getWorld())
	...
	protected boolean help(CommandSender sender, int page) {
		...
		for (Entry<String, Integer> one : commands.entrySet()) {
			...
			if (i >= end)
				break;
			sender.sendMessage(getUsage(one.getKey()) + " - "
					+ Jobs.getLanguage().getMessage("command." + one.getKey() + ".help.info"));
		}
	...
	private static Cmd getCmdClass(String cmd) {
		...
		} catch (ClassNotFoundException | InstantiationException | IllegalArgumentException | IllegalAccessException
	 ...
	 * 
	 * @param player
	 *            - the player of the job
	 * @param job
	 *            - the job we are displaying info about
	 * @param type
	 *            - type of info
	 * @return the message
	 */
	public void jobInfoMessage(CommandSender sender, JobsPlayer player, Job job, String type, int page) {
		...
		if (job.getExpBoost() != 1.0)
			message.append(ChatColor.GOLD
					+ Jobs.getLanguage().getMessage("command.expboost.output.infostats", "%boost%", job.getExpBoost())
					+ "\n");

		if (job.getMoneyBoost() != 1.0)
			message.append(ChatColor.GOLD + Jobs.getLanguage().getMessage("command.moneyboost.output.infostats",
					"%boost%", job.getMoneyBoost()) + "\n");

		if (job.getPointBoost() != 1.0)
			message.append(ChatColor.GOLD + Jobs.getLanguage().getMessage("command.pointboost.output.infostats",
					"%boost%", job.getPointBoost()) + "\n");

		if (Jobs.getGCManager().useDynamicPayment)
			if (job.getBonus() < 0)
				message.append(ChatColor.GOLD + Jobs.getLanguage().getMessage("command.info.help.penalty", "[penalty]",
						(int) (job.getBonus() * 100) / 100.0 * -1) + "\n");
			else
				message.append(ChatColor.GOLD + Jobs.getLanguage().getMessage("command.info.help.bonus", "[bonus]",
						(int) (job.getBonus() * 100) / 100.0) + "\n");
		...
		for (ActionType actionType : ActionType.values()) {
			if (showAllTypes == 1 || type.startsWith(actionType.getName().toLowerCase())) {
				...
				if (info != null && !info.isEmpty()) {
					...
					String myMessage = Jobs.getLanguage()
							.getMessage("command.info.output." + actionType.getName().toLowerCase() + ".none");
		...
		if (sender instanceof Player)
			if (sender.getName().equalsIgnoreCase(player.getUserName()))
				...
				ShowPagination(sender.getName(), pagecount, page,
						"jobs playerinfo " + player.getUserName() + " " + job.getName() + t);
	}
	...
	public static String jobInfoMessage(JobsPlayer player, Job job, ActionType type) {
		...
		for (JobInfo info : jobInfo) {
			...
			if (income != 0.0)
				message.append(Jobs.getLanguage().getMessage("command.info.help.money", "%money%",
						incomeColor + String.format("%.2f", income)));

			if (points != 0.0)
				message.append(Jobs.getLanguage().getMessage("command.info.help.points", "%points%",
						pointsColor + String.format("%.2f", points)));

			if (xp != 0.0)
				message.append(Jobs.getLanguage().getMessage("command.info.help.exp", "%exp%",
						xpColor + String.format("%.2f", xp)));

			if (info.getFromLevel() > 1 && info.getUntilLevel() != -1)
				message.append(Jobs.getLanguage().getMessage("command.info.help.levelRange", "%levelFrom%",
						info.getFromLevel(), "%levelUntil%", info.getUntilLevel()));

			if (info.getFromLevel() > 1 && info.getUntilLevel() == -1)
				message.append(Jobs.getLanguage().getMessage("command.info.help.levelFrom", "%levelFrom%",
						info.getFromLevel()));

			if (info.getFromLevel() == 1 && info.getUntilLevel() != -1)
				message.append(Jobs.getLanguage().getMessage("command.info.help.levelUntil", "%levelUntil%",
						info.getUntilLevel()));
	 ...
	 * 
	 * @param jobProg
	 *            - the job progress of the players job
	 * @return the message
	 */
	public String jobStatsMessage(JobProgression jobProg) {
		String message = Jobs.getLanguage().getMessage("command.stats.output", "%joblevel%",
				...
				Double.toString(Math.round(jobProg.getExperience() * 100.0) / 100.0), "%jobmaxxp%",
				Integer.toString(jobProg.getMaxExperience()));
	...
	public String jobStatsMessage(String jobInfo) {
		...
		if (Jobs.getJob(splited[0]) == null)
			return "";
		String message = Jobs.getLanguage().getMessage("command.archive.output", "%joblevel%",
				Integer.valueOf(splited[1]).toString(), "%getbackjoblevel%", Integer.valueOf(splited[2]).toString(),
				"%jobname%", Jobs.getJob(splited[0]).getChatColor() + splited[0] + ChatColor.WHITE, "%jobxp%",
				Double.toString(Math.round(Double.valueOf(splited[3]) * 100.0) / 100.0));
		return message;
	}
}
