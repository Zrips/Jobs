...
public class expboost implements Cmd {
	...
	public boolean perform(Jobs plugin, CommandSender sender, String[] args) {
		...
		if (args[0].equalsIgnoreCase("reset") && args[1].equalsIgnoreCase("all")) {
			...
			if (found) {
				sender.sendMessage(ChatColor.RED + Jobs.getLanguage()
						.getMessage("command.expboost.output.jobsboostreset", "%jobname%", job.getName()));
				return true;
			}
		}
		...
		sender.sendMessage(ChatColor.GREEN + Jobs.getLanguage().getMessage("command.expboost.output.boostadded",
