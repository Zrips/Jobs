...
public class log implements Cmd {
	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		int max = 10;
		sender.sendMessage(
				Jobs.getLanguage().getMessage("command.log.output.topline", "%playername%", JPlayer.getUserName()));
		for (Log one : logList) {
			HashMap<String, LogAmounts> AmountList = one.getAmountList();
			for (Entry<String, Double> oneSorted : unsortMap.entrySet()) {
				for (Entry<String, LogAmounts> oneMap : AmountList.entrySet()) {
					if (oneMap.getKey().equalsIgnoreCase(oneSorted.getKey())) {
						count++;
						sender.sendMessage(
								Jobs.getLanguage().getMessage("command.log.output.list", "%number%", count, "%action%",
										one.getActionType(), "%item%",
										oneMap.getValue().getItemName().replace(":0", "").replace("_", " ")
												.toLowerCase(),
										"%qty%", oneMap.getValue().getCount(), "%money%", oneMap.getValue().getMoney(),
