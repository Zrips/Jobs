		...
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			...
			public void run() {
				...
				for (Entry<LogAmounts, Double> one : unsortMap.entrySet()) {
					...
					sender.sendMessage(Jobs.getLanguage().getMessage("command.glog.output.list", "%username%",
							one.getKey().getUsername(), "%number%", count, "%action%", info.getAction(), "%item%",
							one.getKey().getItemName().replace(":0", "").replace("_", " ").toLowerCase(), "%qty%",
							one.getKey().getCount(), "%money%", one.getKey().getMoney(), "%exp%",
							one.getKey().getExp()));
					count++;
