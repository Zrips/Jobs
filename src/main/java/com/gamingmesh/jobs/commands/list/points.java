...
public class points implements Cmd {
	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		sender.sendMessage(
				Jobs.getLanguage().getMessage("general.info.toplineseparator", "%playername%", jPlayer.getUserName()));
		sender.sendMessage(Jobs.getLanguage().getMessage("command.points.currentpoints", "%currentpoints%",
				(int) (pointInfo.getCurrentPoints() * 100) / 100D));
		sender.sendMessage(Jobs.getLanguage().getMessage("command.points.totalpoints", "%totalpoints%",
				(int) (pointInfo.getTotalPoints() * 100) / 100D));
