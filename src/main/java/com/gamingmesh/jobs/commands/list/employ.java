...
public class employ implements Cmd {
	...
	public boolean perform(Jobs plugin, CommandSender sender, String[] args) {
		...
		if (jPlayer.isInJob(job)) {
			...
			String message = ChatColor.RED + Jobs.getLanguage().getMessage("command.employ.error.alreadyin",
					"%jobname%", job.getChatColor() + job.getName() + ChatColor.RED);
		...
		try {
			...
			if (player != null)
				player.sendMessage(Jobs.getLanguage().getMessage("command.employ.output.target", "%jobname%",
						job.getChatColor() + job.getName() + ChatColor.WHITE));
