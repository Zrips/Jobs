...
public class moneyboost implements Cmd {
	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		if (args[0].equalsIgnoreCase("reset") && args[1].equalsIgnoreCase("all")) {
			...
			if (found) {
				sender.sendMessage(ChatColor.RED + Jobs.getLanguage()
						.getMessage("command.moneyboost.output.jobsboostreset", "%jobname%", job.getName()));
				return true;
			}
		}
		...
		sender.sendMessage(ChatColor.GREEN + Jobs.getLanguage().getMessage("command.moneyboost.output.boostadded",
				"%boost%", rate, "%jobname%", job.getName()));
		return true;
	}
}
