	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		try {
			...
			if (jPlayer.isInJob(job)) {
				...
				if (player != null) {
					player.sendMessage(Jobs.getLanguage().getMessage("command.promote.output.target", "%jobname%",
							job.getChatColor() + job.getName() + ChatColor.WHITE, "%levelsgained%", levelsGained));
				}
