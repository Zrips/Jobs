	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		try {
			if (jPlayer.isInJob(oldjob) && !jPlayer.isInJob(newjob)) {
				...
				if (player != null) {
					String message = Jobs.getLanguage().getMessage("command.transfer.output.target", "%oldjobname%",
							oldjob.getChatColor() + oldjob.getName() + ChatColor.WHITE, "%newjobname%",
							newjob.getChatColor() + newjob.getName() + ChatColor.WHITE);
