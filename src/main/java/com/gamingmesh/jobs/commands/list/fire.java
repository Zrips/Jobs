...
public class fire implements Cmd {
	...
	public boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		if (!jPlayer.isInJob(job)) {
			String message = ChatColor.RED + Jobs.getLanguage().getMessage("command.fire.error.nojob", "%jobname%",
					job.getChatColor() + job.getName() + ChatColor.RED);
		...
		try {
			...
			if (player != null) {
				String message = Jobs.getLanguage().getMessage("command.fire.output.target", "%jobname%",
